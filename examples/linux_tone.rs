...
fn main() {
    eprintln!("linux_tone is only supported on Linux.");
    std::process::exit(1);
}
...
mod app {
    ...
    fn print_help() {
        println!("Linux tone demo options:");
    ...
    pub fn main() {
        ...
        match backend.create_stream(config) {
            Ok(mut stream) => {
                ...
                println!(
                    "Starting Linux stream: {:?} @ {} Hz, burst {}, tone {} Hz, amp {}, {}s",
            ...
            Err(error) => {
                println!("Could not create Linux stream: {error:?}");
            }
        }
    }
