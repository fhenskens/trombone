...
fn main() {
    eprintln!("windows_bench is only supported on Windows.");
    std::process::exit(1);
}
...
mod app {
    ...
    use trombone::core::metrics::{NegotiatedSampleFormat, NegotiatedShareMode, StreamMetrics};
    ...
    fn print_help() {
        println!("Windows WASAPI benchmark options:");
    ...
    fn run_output(options: BenchOptions, backend: WindowsBackend) -> Result<BenchResult, String> {
    ...
    fn run_input(options: BenchOptions, backend: WindowsBackend) -> Result<BenchResult, String> {
    ...
    fn run_duplex(options: BenchOptions, backend: WindowsBackend) -> Result<BenchResult, String> {
        ...
        let channels = usize::try_from(actual_output.channels.get()).unwrap_or(2);
        let ring = Arc::new(Mutex::new(VecDeque::<f32>::with_capacity(
            (actual_input.sample_rate_hz.get() * actual_input.channels.get()) as usize,
        )));
...

        {
            ...
            output_stream
                .set_render_callback(move |info: CallbackInfo, out: &mut [f32]| {
                    ...
                    out_samples_cb.fetch_add(played, Ordering::Relaxed);
                    zero_filled_cb.fetch_add(zeroed, Ordering::Relaxed);
                })
                .map_err(|error| format!("Could not set output callback: {error:?}"))?;
        }
    ...
    fn print_human(options: BenchOptions, result: &BenchResult) {
        println!(
            "Bench: mode={:?}, backend=WASAPI, seconds={}",
            options.mode, options.seconds
        );
        ...
        if let Some(out) = &result.output {
            ...
            let out_format = out
                ...
                .unwrap_or("unknown");
            println!(
                "Output: callbacks={}, samples={}, xruns={}, observed_frames_per_callback={}, mode={}, format={}",
                out.callbacks,
                out.samples,
                out.metrics.xrun_count,
        ...
        if let Some(input) = &result.input {
            ...
            let in_format = input
                ...
                .unwrap_or("unknown");
            println!(
                "Input: callbacks={}, samples={}, xruns={}, observed_frames_per_callback={}, mode={}, format={}",
                input.callbacks,
                input.samples,
    ...
    fn share_mode_label(mode: NegotiatedShareMode) -> &'static str {
        match mode {
            NegotiatedShareMode::Shared => "shared",
            NegotiatedShareMode::Exclusive => "exclusive",
        }
    }

    fn sample_format_label(format: NegotiatedSampleFormat) -> &'static str {
        match format {
            NegotiatedSampleFormat::F32 => "f32",
            NegotiatedSampleFormat::I16 => "i16",
        }
    }
